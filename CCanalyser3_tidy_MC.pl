...
=head1 SYNOPSIS
  ...
  This script uses a sam file as input, which can generated as follows:
  1.	Perform adaptor trimming of the raw fastq files if this has not been performed by the sequencer.
	I tend to use trim_galore to do this with the paired settings.
	For example:
	trim_galore --paired Paired_end_1.fastq Paired_end_2.fastq

  2.	If you are using 150 bp paired end reads from the Miseq then the overlapping reads need to be merged.  I use FLASH to do this.
	For example:
	flash --interleaved-output Paired_end_1.fq Paired_end_2.fq
  ...
  3.	If you sequence the data with reads that are shorter than the fragments then you may not need to merge the reads with flash.
  ...
  4.	I normally align the fastq files with bowtie using one processor only because it is crucial that the reads are kept in strict order,
	...
	I tend to use m2, best and strata Ð but this is partly because m2 is needed otherwise all of the reads for alpha globin will be thrown (because of the gene duplication).
	I would use 'best' so that each read can only align once otherwise each read could be counted many times. 
	For example:
	bowtie -p 1 -m 2 --best --strata --sam --chunkmb 256 --sam /databank/indices/bowtie/mm9/mm9 Sample_REdig.fastq Sample_REdig.sam
  ...
  The script requires the perl modules Data::Dumper; Getopt::Long and Pod::Usage. It also needs wigToBigWig.
  ...
  This script will create a subdirectory (of the directory the script is in) named after the sample.
...
=head1 EXAMPLE
 ...
 CCanalyser3_tidy.pl -f input_sam_file.sam -o input_oligo_file.txt -r input_restriction_enzyme_coordinates_file.txt -s short_sample_name -pf public_folder -pu public_url
...
=head1 AUTHOR
 ...
 Last release modified based on CC2 release by Jelena Telenius October 2015 (CC3)
 This release modified by Mingyang Cai June 2016 (MC)
...
# Creates files for the report and a fastq file for unaligned sequences to go into
...
open( DPNFH, "$restriction_enzyme_coords_filename_path" ) or die "Cannot find restriction enzyme data file $restriction_enzyme_coords_filename_path: $!";
...
while ( my $line = <INFH> )    #loops through all the lines of the sam file
{
	...
	$counters{"05 Discarded fragments with wrongly parsing SAM file name:"}++;
	next;
    }
    else
	...
	if ( ( $bitwise & 4 ) == 4 ) {
	    ...
	    $counters{"06e Fragments with CIGAR strings failing to parse (containing introns or indels) in SAM file:"}++;
	...
	# The rest of the reads are now parse-able :
	    ...
	    $counters{"07 Mapped fragments (parse-able) in SAM file:"}++;
...
print REPORTFH "Script finished at: $mday/$mon/$year $hour:$min \n";

print REPORTFH
"Analysed reads (last fragments): $analysedReads, not-last-fragments :$notlastFragments \n";
...
sub readAnalysisLoop {
	...
	# Here the old duplicate filter. this over-rules the above "VS2 duplicate filter".
        $data{$analysis_read}{"coord string"} = join( "_",
            sort { $a cmp $b } @{ $data{$analysis_read}{"coord array"} } )
                ...
                for ( my $readno = 0; $readno < 4; $readno++ )    # loops through up to 4 fragments in each paired end read
                {
                    ...
                    if ( defined $data{$analysis_read}{$pe}{$readno}{"type"} ) {
                        ...
                        if ( $data{$analysis_read}{$pe}{$readno}{"type"} eq "capture" ) {
                            $counters{"16b Capture fragments (After PCR duplicate removal):"}++;
                        }
                        if ( $data{$analysis_read}{$pe}{$readno}{"type"} eq "proximity exclusion" ) {
                            $counters{"16c Proximity exclusion fragments (After PCR duplicate removal):"}++;
                        }
                        if ( $data{$analysis_read}{$pe}{$readno}{"type"} eq "reporter" ) {
                            $counters{"16d Reporter fragments (After PCR duplicate removal):"}++;
                        }
                    }
                }
            }
...
sub binary_search {
    ...
    for ( my $i = 0 ; $i < 99 ; $i++ ) {
        ...
        if ( ( $$arrayref[$mid_search] <= $value )
            and ( $$arrayref[ $mid_search + 1 ] > $value )
          )    # maps the mid point of the read to a fragment
        {
            if (    ( $$arrayref[$mid_search] <= $start + 2 )
                and ( $$arrayref[ $mid_search + 1 ] >= $end - 2 )
              ) # checks the whole read is on the fragment +/-2 to allow for the dpnII overlaps
            {
                return ( $$arrayref[$mid_search],
                    $$arrayref[ $mid_search + 1 ] - 1 );
            }
            else {
                $$counter_ref{
"25ee Binary search error - fragment overlapping multiple restriction sites:"
                }++;
                return ( "error", "error" );
            }
        }
...
sub frag_to_migout {
    ...
    if ( keys %{ $$hashref{$fragtype}{$capture} } ) {
        ...
        foreach my $storedChr (
            sort keys %{ $$hashref{$fragtype}{$capture} }
          ) #{ $hash{$b} <=> $hash{$a} }  { {$hash{$storedChr}{$b}} <=> {$hash{$storedChr}{$a}} }
        {
            foreach my $storedposition (
                sort keys %{ $$hashref{$fragtype}{$capture}{$storedChr} } )
            {
                print MIGOUTPUT
                  "chr$storedChr\t$version\t$capture\t$storedposition\t"
                  . $$hashref{$fragtype}{$capture}{$storedChr}{$storedposition}
                  {"end"} . "\t"
                  . $$hashref{$fragtype}{$capture}{$storedChr}{$storedposition}
                  {"value"}
                  . "\t+\t0\t.\n";
          ...
          ) #{ $hash{$b} <=> $hash{$a} }  { {$hash{$storedChr}{$b}} <=> {$hash{$storedChr}{$a}} }
        {
                ...
                sort keys %{ $$hashref{$fragtype}{$capture}{$storedChr} } )
            {
                ...
                my $startsamm = $storedposition + (
                    (
                        $$hashref{$fragtype}{$capture}{$storedChr}
                          {$storedposition}{"end"} - $storedposition
                    ) / 2
